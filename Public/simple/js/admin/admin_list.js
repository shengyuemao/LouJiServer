...
function show_data(list) {
	...
	for(var i in list) {
		...
		item_str += '<td class="first style1">' + item.username + '</td>';
		item_str += '<td>' + item.time + '</td>';
		item_str += '<td class="last"><a href="#" class="btn_del" id="btn_del_' + item.uid + '">删除</a></td>';
		item_str += '</tr>';
		$(".listing > tbody").append(item_str);
	}
	...
	$(".btn_del").click(function() {
		if(confirm("确认删除吗？")) {
			...
			$.getJSON(
				"ajax_admin_del?uid=" + id + "&random=" + Math.random(),
				function(data) {
					alert(data.desc);
					do_page();
				}
...
$(function() {
	...
	$("#add_btn").click(function() {
		$("#center-column").load("/first/Public/simple/admin_templates/admin_add.html?random=" + Math.random());
		return false;
	});
